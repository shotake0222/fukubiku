...
- **MindARターゲット画像**: 画像をアップロードすると、オブジェクト管理や注文編集と同様に
  ブラウザ上で自動的に`.mind`ファイルへコンパイルされます。
...
3. アイテム編集画面で「+ 発火条件を追加」し、ARモード（A-Frame / MindAR画像認識 / MindAR顔認識 / GPS）を
   ...
   （1アイテムに発火条件はいくつでも追加可能）
4. 各発火条件の「+ オブジェクトを追加」で表示オブジェクトを追加（プリセットから選ぶか独自ファイルをアップロード）。
   1つの発火条件に複数オブジェクトを配置し、位置・大きさ・回転を個別に調整できる
5. アイテムの状態を「公開準備完了」に切り替えると、クライアント提供URL（`https://app.attend-ar.com/a/xxxxxxxxxx`）が有効になる
