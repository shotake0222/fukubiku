...
export default async function AttendRalliesPage() {
  ...
  const [{ data: projects }, { data: spotRows }, { data: participantRows }] = await Promise.all([
    projectIds.length
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-100 text-left text-slate-500">
            <tr>
              ...
              <th className="px-4 py-2">参加者URL</th>
            ...
            {rallyList.map((r) => (
              <tr key={r.id}>
                ...
                <td className="px-4 py-2">
                  <code className="text-xs break-all">{`${siteOrigin}/r/${r.hash}`}</code>
                </td>
                <td className="px-4 py-2">
