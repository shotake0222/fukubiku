...
//
// AR.jsのa-marker座標系は「マーカー面=XZ平面 / 法線=+Y」で、モデルの+Y(上)が
// マーカーから垂直に生えていく向きになる。プリセットの.glbは正面が+Zを向く
// 通常のY-up構成のため、無回転だとマーカー面に寝転んだ状態になり、実機では
// 円盤を真横から見た「平たい板」のように見えてしまっていた。
// X軸まわりに-90度回すと、正面(+Z)がマーカーの法線(+Y)＝見ている側を向き、
// かつ上下も正立する。これは旧実装(index.html)の
//   <a-video position="0 0 0" rotation="-90 0 0" ...>
// と同じ値で、同じマーカーで実績のある向き。
// ローカルでマーカー座標系を再現してレンダリング比較し、
//   "0 180 0"   → 円盤を真横から見た状態(実機の症状と一致)
//   "-90 0 0"   → 正面が正立してこちらを向く  ← 採用
//   "90 0 0"    → 背面が見える
//   "-90 0 180" → 正面だが上下逆
