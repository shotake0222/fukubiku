...
// 表示オブジェクト(3Dモデル/動画/画像)にrotation未指定の場合の既定の向き。
//
// AR.jsの軸変換をライブラリ実装で確認したところ、
//   projectionAxisTransform = makeRotationY(π) × makeRotationZ(π)
// でY軸とZ軸を反転しており、マーカー座標系は
//   「マーカー面 = XY平面 / 法線(視聴者側) = +Z」
// になる。プリセットの.glbも動画の平面も正面が+Zを向いて作られているため、
// 無回転のままでマーカーの正面を向く。
//
// 以前はここに "-90 0 0" を入れていたが、それではオブジェクトがマーカー面に
// 寝てしまい、正面から見ると線にしか見えず「マーカーを傾けた時だけ薄く見える」
// 状態になっていた(実機で報告された症状と一致)。
//
// 個別に合わない場合は管理画面(表示オブジェクト管理)の向き欄、または
// URLの ?rot=x y z で上書きできる。?debug=1&rotgrid=1 で候補を並べて比較できる。
export const DEFAULT_MODEL_ROTATION = "0 0 0";
