...
//
// AR.jsに実際にマーカーを検出させて、マーカー座標系の各軸がカメラから見て
// どこを向くかを実測した結果:
//   X軸 = ( 1,  0,  0)  画面の右
//   Y軸 = ( 0,  0,  1)  カメラの方向(視聴者側) ← これが法線
//   Z軸 = ( 0, -1,  0)  画面の下
// つまり「マーカー面 = XZ平面 / 法線 = +Y」。
// 正面が+Zを向いて作られたモデルや平面は、X軸まわりに-90度回すと
// 正面(+Z)が法線(+Y)＝視聴者側を向く。旧実装(index.html)の
//   <a-video position="0 0 0" rotation="-90 0 0" scale="3 3 3">
// と同じ値であり、実測でも -90 0 0 の板だけがはっきり見え、
// 0 0 0 の板は真横を向いて見えないことを確認済み。
//
// 補足: 画面の「上」はマーカー座標系の -Z。位置を上げたい場合はZを負にする。
//
